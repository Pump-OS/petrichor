...
function drawOffScreen() {
  ...
  const rg = ctx.createRadialGradient(CW/2, CH/2, 0, CW/2, CH/2, CH * 0.7);
  rg.addColorStop(0, 'rgba(30,30,30,0.15)');
...
function powerOnTV() {
  ...
  if (led) led.classList.remove('led-off');
...
window.powerOnTV = powerOnTV;

// ── SWITCH CHANNEL ────────────────────────────────────────────────
function switchTo(idx) {
  if (!tvPowered || switching || deepDiveActive) return;
  switching    = true;
  currentCh    = ((idx % CHANNELS.length) + CHANNELS.length) % CHANNELS.length;
  staticBurst  = 1.0;

  visitedChannels.add(currentCh);
  ...
  setTimeout(() => {
    ...
    if (visitedChannels.size === CHANNELS.length && !deepDiveTriggered) {
      deepDiveTriggered = true;
      deepDiveActive = true;
      clearInterval(autoTimer);
      setTimeout(() => {
        if (typeof window.onAllChannelsVisited === 'function') {
          window.onAllChannelsVisited();
        }
      }, 800);
    }
  }, 500);
...
document.addEventListener('keydown', e => {
  if (!tvPowered || deepDiveActive) return;
